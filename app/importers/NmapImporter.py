...
from parsers.Parser import parseNmapReport, MalformedXmlDocumentException, Parser
...
class NmapImporter(QtCore.QThread):
    ...
    def __tryParseNmapXml(self) -> Optional[Parser]:
        self.tsLog(f"Parsing nmap xml file: {self.filename}")
        try:
            report = parseNmapReport(self.filename)
    ...
    def run(self):
        # -- db access start
        with DatabaseSession(self.db) as session, ProgressTracker(self.updateProgressObservable) as progressTracker:
            nmapReport = None
            startTime = time()

            nmapReport = self.__tryParseNmapXml()
            if nmapReport is None:
                return

            try:
                s = nmapReport.getSession()  # nmap session info
                if s:
                    n = nmapSessionObj(self.filename, s.startTime, s.finish_time, s.nmapVersion, s.scanArgs,
                                       s.totalHosts, s.upHosts, s.downHosts)
                    session.add(n)
                ...
                for h in allHosts:  # create all the hosts that need to be created
                    ...
                    progressTracker.updateProgress(int(totalprogress))
                ...
                for h in allHosts:  # create all OS, service and port objects that need to be created
                    ...
                    for p in all_ports:  # parse the ports
                        ...
                        if not (s is None):  # check if service already exists to avoid adding duplicates
                            ...
                            # port=str(p.portId)))
                            # db_service = session.query(serviceObj).filter_by(name=s.name)
                            # .filter_by(product=s.product).
                ...
                for h in allHosts:  # update everything
                    ...
                    for p in h.all_ports():
                        s = p.getService()
                        if not (s is None):
                            # db_service = session.query(serviceObj)
